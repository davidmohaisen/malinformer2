...
packedDataPath = "../UPX/staticAnalysis/upxPacked/symbols/"
f = open("../UPX/staticAnalysis/Packed-fam_malw_MalwareList.pickle","rb")
labels = pickle.load(f)
x_test_p = []
y_test_p = []
for sample in os.listdir(packedDataPath):
    file_to_scan = packedDataPath+sample
    f = open(file_to_scan,"rb")

    dataSample = pickle.load(f)
    x_test_p.append(dataSample)
    sample2 = sample.replace("VirusShare_","")
    sample2 = sample2.replace(".pickle","")
    sample = sample.replace(".pickle","")
