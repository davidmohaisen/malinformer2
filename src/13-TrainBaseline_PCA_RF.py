...
ctr_files = {}
# families = set()
for file in os.listdir('../pca/'):
    num = float(file.rsplit("-")[3].rsplit(".p")[0])
    # print(num)
    # families.add(file.rsplit("-")[1])
    if num not in ctr_files:
        ctr_files[num] = set()
        ctr_files[num].add('../pca/'+file)
    else:
        ctr_files[num].add('../pca/'+file)
...
for num in ctr_files:
    ...
    for file in ctr_files[num]:
        ...
        for j in range(len(Data)):
            x.append(Data[j])
            y.append((int(int(file.rsplit("-")[2])/100)))
    # x = np.asarray(x)
    # y = np.asarray(y)
    # print(x.shape)
    # print(y.shape)
    ...
    print(num,x_train.shape[1],score)
