...
for family in families:
    ...
    f = open("../UPX/transformedOnTop20000/"+family+"-20000-frequency-Train.pickle","rb")
    ...
    f = open("/media/ahmed/HDD/FamilyAnalysis/topK_dynamic/transformedOnTopKFeats/"+family+"-20000-frequency-Test.pickle","rb")
    partial_x = pickle.load(f)
    partial_x = np.asarray(partial_x)
    y_partial = [families.index(family)]*len(partial_x)
    if len(x_test) == 0:
    ...
    f = open("../UPX/transformedOnTop20000_unpacked/"+family+"-20000-frequency-Test.pickle","rb")
    partial_x = pickle.load(f)
    partial_x = np.asarray(partial_x)
    y_partial = [families.index(family)]*len(partial_x)
    ...
    f = open("../UPX/transformedOnTop20000/"+family+"-20000-frequency-Test.pickle","rb")
    partial_x = pickle.load(f)
    partial_x = np.asarray(partial_x)
    y_partial = [families.index(family)]*len(partial_x)
