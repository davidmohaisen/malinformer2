...
for i in np.arange(4000,4500,500):
    ...
    for family in families:
        ...
        f = open("../topK_dynamic/transformedOnTop20K_ExceptTsunamiGenericaFeats/"+family+"-"+str(i)+"-frequency-Unpacked.pickle","rb")
        partial_x = pickle.load(f)
        partial_x = np.asarray(partial_x)
        y_partial = [families.index(family)]*len(partial_x)
        if len(x_test) == 0:
    ...
    plt.savefig("../Heatmaps/HM-DynamicTopKUnpacked_except.pdf")
    plt.clf()
    plt.close()
